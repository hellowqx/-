...
class CookiesPool:
    ...
    def get_from_db(self):
        '''
        从数据库中读取已有的cookies，并检验其有效性
        :return:
        '''
        strsql = "select * from sinacookies"
    ...
    def gen_cookies(self,loginname,password):
        if loginname not in self.id_list:
            try:
                ...
                #验证码
                verify = self.driver.find_element_by_xpath('//div[@class="info_list verify clearfix"]')
                print("verify:",verify)
                print('verify=', verify.get_attribute('style') == 'display: none;')
                if verify != None and verify.get_attribute('style') != 'display: none;':
                    elem_verify = self.driver.find_element_by_name('verifycode')
                    #print('verify:',veriy)
                    verifycode = input("输入验证码：")
                    elem_verify.clear()
                    elem_verify.send_keys(verifycode)

                # 点击登录按钮
                time.sleep(3)
                self.driver.find_element_by_xpath('//div[@id="pl_login_form"]/div/div[3]/div[6]/a').click()
                time.sleep(5)
                cookies = self.driver.get_cookies()  # 获取登录之后的cookies信息
                print('cookies type:',type(cookies))
                print('cookies values:',cookies)
                home_url = self.driver.current_url  # 获取登陆之后的url
                print('url:',home_url)
    ...
    def check_cookies(self,username,cookies,home_url):
        ...
        '''
        try:
            cookies = json.loads(cookies)  #cookie字典
            print('cookies type:',type(cookies))
        except TypeError:
